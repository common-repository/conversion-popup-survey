...
$form = array(
	...
	'pops_general_margin_tb' => '10',
	'pops_general_margin_lr' => '10',
...
if (isset($_POST['form_submit']) && $_POST['form_submit'] == 'yes')
{
	...
	check_admin_referer('pops_add_new_design');
	...
	{
		$sql = $wpdb->prepare(
			"INSERT INTO `".POPS_DESIGN_TABLE."`
			(`name`, `design_data`, `date_created`)
			VALUES(%s, %s, NOW())",
			array($form['pops_design_name'], serialize($form))
		);
		$wpdb->query($sql);

		$pops_success = 'Design successfully saved.';
...
?>
	...
	<form method="post" action="<?php echo get_option('siteurl'); ?>/wp-admin/admin.php?page=pops_design&amp;sp=add_new">
		<div>
			...
			<?php wp_nonce_field('pops_add_new_design'); ?>
		</div>
	    ...
	    <p class="submit"><input type="button" class="button-secondary" id="pops_preview_design" value="Preview Design" /> <input type="submit" class="button-primary" value="Save Design" /></p>
	</form>
</div>
